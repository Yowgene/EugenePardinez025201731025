...
//not included in the assignment
    try {
        ...
        if ($ref) { //if/else statement to check if there is a reference input
            $stmt = $pdo->prepare("SELECT * FROM users WHERE id = :ref"); //preparing the statement
            $stmt->execute(['ref' => $ref]); //using ref to execute the search
            $portfolio = $stmt->fetchAll(PDO::FETCH_ASSOC); //fetch all results
            echo json_encode($portfolio); //encode in json format
        } else { //if theres no reference
            $stmt = $pdo->query("SELECT * FROM users"); //statement and query search
